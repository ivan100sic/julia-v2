...
vec4 oldBluePalette(int steps) {
    const float COLOR_SCALE = 12.75f;

    float r = float(min(max(steps, 0), 20));
    float g = float(min(max(steps - 20, 0), 20));
    float b = float(min(max(steps - 40, 0), 20));
    
    return vec4(vec3(b, g, r) / COLOR_SCALE, 1.0f);
}
...
float hueCurve(float hue) {
    hue = mod(hue, 1.f);
    // float b = max(0.f, (cos(-hue * 2.f * PI) + 0.5f) / 1.5f);
...
const vec3 COLOR_ADJUSTMENT = vec3(
    0.9f,
    0.7f,
    1.0f
);
...
vec4 huePalette(float steps, float hue) {
    ...
    return vec4(sigma3normalized(c + sf), 1.f);
}
